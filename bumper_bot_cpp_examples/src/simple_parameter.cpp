...
class SimpleParameter : public rclcpp::Node
{
    ...
    SimpleParameter() : Node("simple_parameter")
    {
        ...
        declare_parameter<std::string>("simple_string_param", "Honour_Obed");
        param_callbak_handle_ = add_on_set_parameters_callback(std::bind(&SimpleParameter::paramChangeCallback, this, _1));
    }

private:
OnSetParametersCallbackHandle::SharedPtr param_callbak_handle_;
...
rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> &parameters)
{
    ...
    for (const auto& param : parameters)
    {
        ...
        {
            RCLCPP_INFO_STREAM(get_logger(), "Parameter simple_int_param has been changed, New value: " << param.as_int());
            result.successful = true;
        }
        if(param.get_name() == "simple_string_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
        {
            RCLCPP_INFO_STREAM(get_logger(), "Parameter has been successfully changed, New value: " << param.as_string());
            result.successful = true;
        }
